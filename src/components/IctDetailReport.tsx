...
import {
  Monitor, Users, Mail, FolderOpen, ShieldAlert,
  Wifi, Cloud, Globe, HardDrive, TrendingUp, TrendingDown,
  CheckCircle2, Clock, AlertCircle, ChevronUp, ChevronDown,
  Ticket, CheckCircle, AlarmClock, AlertTriangle, Wrench,
} from "lucide-react";
import {
...
const systems = [
  { name: "Skill Matching",  status: "Active",     feature: 12, completed: 10, inProgress: 1, pending: 1, pct: 83 },
  { name: "Survey",          status: "Active",     feature: 18, completed: 12, inProgress: 4, pending: 4, pct: 67 },
  { name: "Skill Voucher",   status: "Processing", feature: 31, completed: 18, inProgress: 8, pending: 5, pct: 58 },
  { name: "PMS",             status: "Inactive",   feature: 19, completed: 11, inProgress: 7, pending: 1, pct: 58 },
  { name: "Joint Training",  status: "Active",     feature: 13, completed: 7,  inProgress: 3, pending: 3, pct: 54 },
  { name: "E-Learning",      status: "Processing", feature: 28, completed: 18, inProgress: 5, pending: 5, pct: 64 },
];
...
const kpiCards = [
  { icon: Monitor,       color: BLUE,      label: "System Uptime",     value: "99.5%", sub: "+0.3%",          up: true  },
  { icon: Users,         color: "#10b981", label: "Active Users",      value: "185",   sub: "+12 this month", up: true  },
  { icon: Mail,          color: "#f59e0b", label: "Email Accounts",    value: "47",    sub: "Managed",        up: null  },
  { icon: FolderOpen,    color: "#6366f1", label: "System Projects",   value: "6",     sub: "In progress",    up: null  },
  { icon: ShieldAlert,   color: "#ef4444", label: "Security Status",   value: "2",     sub: "Warnings",       up: false },
  { icon: Ticket,        color: "#8b5cf6", label: "Open Tickets",      value: "34",    sub: "+5 today",       up: false },
  { icon: CheckCircle,   color: "#10b981", label: "Resolved Tickets",  value: "128",   sub: "This month",     up: true  },
  { icon: AlarmClock,    color: "#f97316", label: "Avg Response Time", value: "1.4h",  sub: "-12 min",        up: true  },
  { icon: AlertTriangle, color: "#ef4444", label: "Critical Issues",   value: "3",     sub: "Urgent",         up: false },
  { icon: Wrench,        color: "#06b6d4", label: "Maintenance Tasks", value: "11",    sub: "Scheduled",      up: null  },
];
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`text-xs font-semibold px-3 py-1 rounded-full ${STATUS_STYLES[status] ?? "bg-gray-100 text-gray-500"}`}>
      {status}
    </span>
  );
}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${value}%`, background: color, transition: "width 0.6s ease" }} />
      </div>
      <span className="text-sm font-bold w-10 text-right" style={{ color }}>{value}%</span>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 p-8 font-sans text-gray-800">
        ...
        {kpiCards.map((k, i) => (
          <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: `${k.color}15` }}>
                <k.icon size={20} style={{ color: k.color }} />
              </div>
              {k.up !== null ? (
                <span className={`flex items-center gap-1 text-xs font-bold ${k.up ? "text-emerald-500" : "text-red-500"}`}>
                  {k.up ? <TrendingUp size={11} /> : <TrendingDown size={11} />}
                  {k.sub}
                </span>
              ) : (
                <span className="text-xs font-medium text-gray-400">{k.sub}</span>
              )}
            </div>
            {/* Value */}
            <p className="text-4xl font-bold leading-none mb-2" style={{ color: k.color }}>{k.value}</p>
            {/* Label */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <h3 className="font-bold text-base mb-1" style={{ color: BLUE }}>System Usage Trend</h3>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <h3 className="font-bold text-base mb-1" style={{ color: BLUE }}>System Development Activity</h3>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <h3 className="font-bold text-base mb-1" style={{ color: BLUE }}>Support Ticket Trend</h3>
          ...
          <div className="flex gap-2 mt-4 flex-wrap">
            {[
              ...
              <div key={p.label} className="flex-1 min-w-[60px] rounded-xl px-3 py-2 text-center"
      ...
      <div className="flex gap-5 mb-8 flex-wrap xl:flex-nowrap">
        ...
        {/* Table */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex-[7] min-w-0">
          <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h3 className="font-bold text-base" style={{ color: BLUE }}>System Development Status</h3>
              <p className="text-gray-400 text-sm mt-0.5">All active and monitored systems</p>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-sm">
                  {[
                    ["name","System"], ["status","Status"], ["feature","Feature"],
                    ["completed","Completed"], ["inProgress","In Progress"],
                    ["pending","Pending"], ["pct","Completion %"],
                  ].map(([col, label]) => (
                    <th key={col} onClick={() => handleSort(col)}
                      ...
                      <span className="flex items-center gap-1">
                        {label}
                        {sortCol === col ? (sortDir === "asc" ? <ChevronUp size={13} /> : <ChevronDown size={13} />) : null}
                      </span>
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {systems.map((s, i) => (
                  <tr key={i} className="border-t border-gray-50 hover:bg-blue-50/30 transition-colors">
                    <td className="px-5 py-4 font-semibold text-gray-800 text-sm">{s.name}</td>
                    <td className="px-5 py-4"><StatusBadge status={s.status} /></td>
                    <td className="px-5 py-4 font-bold text-sm" style={{ color: BLUE }}>{s.feature}</td>
                    <td className="px-5 py-4 font-bold text-sm text-emerald-600">{s.completed}</td>
                    <td className="px-5 py-4 font-bold text-sm text-amber-500">{s.inProgress}</td>
                    <td className="px-5 py-4 font-bold text-sm text-red-400">{s.pending}</td>
                    <td className="px-5 py-4 min-w-[150px]"><PctBar value={s.pct} /></td>
                  </tr>
                ))}
                {/* Totals row */}
                <tr className="border-t-2 border-gray-200 bg-gray-50">
                  <td className="px-5 py-4 font-bold text-sm text-gray-700">Total</td>
                  <td className="px-5 py-4" />
                  <td className="px-5 py-4 font-bold text-sm" style={{ color: BLUE }}>{totals.feature}</td>
                  <td className="px-5 py-4 font-bold text-sm text-emerald-600">{totals.completed}</td>
                  <td className="px-5 py-4 font-bold text-sm text-amber-500">{totals.inProgress}</td>
                  <td className="px-5 py-4 font-bold text-sm text-red-400">{totals.pending}</td>
                  <td className="px-5 py-4 min-w-[150px]"><PctBar value={totals.pct} /></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex-[3] min-w-[240px]">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h3 className="font-bold text-base" style={{ color: BLUE }}>Digital Infrastructure</h3>
          </div>
          ...
          <div className="mt-5 p-4 rounded-xl border border-emerald-100 bg-emerald-50/50">
            <p className="text-emerald-600 text-sm font-semibold mb-2">Overall Health</p>
            <div className="flex items-center gap-3">
