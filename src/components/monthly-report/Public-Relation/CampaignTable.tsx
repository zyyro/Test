...
type Col = "name" | "channel" | "reach" | "application" | "convRate" | "costPerCampaign";
...
export default function CampaignTable() {
  ...
  const totals = {
    ...
    convRate:    +(campaigns.reduce((a, c) => a + c.convRate, 0) / campaigns.length).toFixed(2),
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
        <div>
          <h3 className="font-bold text-lg" style={{ color: BLUE }}>Campaign Performance</h3>
          <p className="text-gray-400 text-sm mt-0.5">All campaigns this period</p>
        </div>
          ...
          <tbody>
            {sorted.map((c, i) => (
              <tr key={i} className="border-t border-gray-50 hover:bg-blue-50/30 transition-colors">
                <td className="px-6 py-4 font-semibold text-sm" style={{ color: BLUE }}>
                  {c.name}
                </td>
                <td className="px-6 py-4">
                  <span className={`text-xs font-semibold px-3 py-1 rounded-full ${CHANNEL_COLORS[c.channel] ?? "bg-gray-100 text-gray-500"}`}>
                    {c.channel}
                  </span>
                </td>
            ...
            <tr className="border-t-2 border-gray-200 bg-gray-50">
              <td className="px-6 py-4 font-bold text-sm text-gray-700">Total / Avg</td>
              <td className="px-6 py-4 text-sm text-gray-400">{campaigns.length} channels</td>
              <td className="px-6 py-4 font-bold text-sm" style={{ color: BLUE }}>
